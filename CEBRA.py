...
# -------------------- ساخت ماتریس عصبی (تعداد نمونه‌ها × نورون‌ها) --------------------
n_samples = len(cursor_times)
neural = np.zeros((n_samples, n_neurons), dtype=np.float32)

# تطبیق هر اسپایک با نزدیک‌ترین زمان cursor
# (اگر زمان‌ها به اندازه کافی نزدیک باشند؛ این روش ساده است)
# برای سرعت، می‌توانیم از np.searchsorted استفاده کنیم
indices = np.searchsorted(cursor_times, spike_times, side='right') - 1
# حذف اسپایک‌هایی که قبل از اولین زمان cursor یا بعد از آخرین آن هستند
valid = (indices >= 0) & (indices < n_samples)
indices = indices[valid]
spike_units = spike_units[valid]

# شمارش اسپایک‌ها در هر زمان برای هر نورون
for idx, unit in zip(indices, spike_units):
    neural[idx, unit] += 1

print("Shape of neural matrix:", neural.shape)      # (10178, 120)
print("Shape of cursor velocity:", cursor_vel.shape) # (10178, 2)

# -------------------- استفاده از ماسک‌های train/test (اختیاری) --------------------
# اگر ماسک‌ها معتبر باشند، می‌توانیم داده را بر اساس آن‌ها تقسیم کنیم
if cursor_train_mask.sum() > 0 and cursor_test_mask.sum() > 0:
    ...
    # در غیر این‌صورت از split_data با نسبت 80/20 استفاده می‌کنیم
...
class RobustDecoder(nn.Module):
    def __init__(self, input_dim):
        ...
        self.net = nn.Sequential(
            ...
            nn.Linear(128, 64),
            nn.ReLU(),
            nn.Linear(64, 2)  # خروجی ۲ بعدی (سرعت x و y)
        )
...
def train_decoder_optimized(emb_train, emb_test, label_train, label_test, epochs=5000, lr=0.01):
    # نرمال‌سازی برچسب‌ها (هر بعد جداگانه)
    y_train = label_train
    y_test = label_test
